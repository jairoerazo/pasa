...
class ProductAgent:
    ...
    def search_product(self, queries):
        while queries:
            with self.lock:
                query, self.root.child[query] = queries.pop(), []
            pre_asins, searched_products = google_search_asin(query, self.search_papers), []
            for asin in pre_asins:
                self.lock.acquire()
                if asin not in self.root.extra["touch_ids"]:
                    self.root.extra["touch_ids"].append(asin)
                    self.lock.release()
                    product = search_product_by_asin(asin)
                    if product is not None:
                        searched_products.append(product)
                else:
                    self.lock.release()
            select_prompts = [
                self.prompts["get_selected"].format(
                    title=prod["title"],
            ...
            with self.lock:
                for score, prod in zip(scores, searched_products):
